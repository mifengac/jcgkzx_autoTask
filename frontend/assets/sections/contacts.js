...
function renderDirectoryFilters(app) {
  ...
  return `
    <form id="contact-directory-filter-form">
      <div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(12rem, 1fr));">
        <div>
      ...
      <div role="group">
        ...
        <button class="secondary" type="button" data-action="contact-directory-reset">重置筛选</button>
        <button class="outline" type="button" data-action="contact-create">新建联系人</button>
      </div>
    </form>
  `;
}
...
function renderPhoneRow(phone, index, readonly) {
  ...
  return `
    <div data-phone-row>
      <div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(12rem, 1fr)); align-items:end;">
        <div>
          <label>原始号码</label>
        ...
        ${readonly ? "" : `<div><label>操作</label><button class="outline" type="button" data-action="contact-phone-remove">移除号码</button></div>`}
      </div>
    </div>
  `;
}
...
function renderEditor(app) {
  ...
  return `
    ...
    ${readonly ? `<div>该联系人来自导入数据源，只能查看，不能在此页面修改。</div>` : ""}
    <form id="contact-editor-form">
      ...
      <div>
        <div style="margin-bottom:12px;">
          ...
          ${readonly ? "" : `<div><button class="outline" type="button" data-action="contact-phone-add">新增号码</button></div>`}
        </div>
        <div id="contact-phone-list">
          ${phones.map((phone, index) => renderPhoneRow(phone, index, readonly)).join("")}
        </div>
      </div>
      <div role="group">
        ${readonly ? "" : `<button type="submit">${contact ? "保存联系人" : "创建联系人"}</button>`}
        <button class="secondary" type="button" data-action="contact-create">新建联系人</button>
        <button class="outline" type="button" data-action="contact-directory-open">返回目录</button>
      </div>
    </form>
  `;
}
