      ...
      ${app.state.tasks.map((task) => {
        ...
        return `
          <article>
            ...
            <div role="group">
              <button class="outline" type="button" data-action="task-select-card" data-id="${task.id}">选中</button>
              <button class="outline" type="button" data-action="task-edit" data-id="${task.id}">编辑</button>
              <button class="outline" type="button" data-action="${task.enabled ? "task-disable" : "task-enable"}" data-id="${task.id}">${task.enabled ? "停用" : "启用"}</button>
              <button class="outline" type="button" data-action="task-delete" data-id="${task.id}">删除</button>
            </div>
          </article>
        `;
      }).join("")}
  ...
  return `
    <form id="task-form">
      ...
      <div role="group">
        ...
        <button class="secondary" type="button" data-action="task-reset">新建任务</button>
        <button class="outline" type="button" data-action="task-run-dry" ${app.state.selectedTaskId ? "" : "disabled"}>演练</button>
        <button class="contrast" type="button" data-action="task-run-live" ${app.state.selectedTaskId ? "" : "disabled"}>立即执行</button>
      </div>
    </form>
  `;
}
...
function renderTaskRuleForm(app) {
  ...
  return `
    <div>${task ? `当前任务: ${escapeHtml(task.task_name)}` : "请先选中任务，再维护接收规则。"}</div>
    <form id="task-rule-form">
      <div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(16rem, 1fr));">
        ${taskSelector(app)}
        <div>
      ...
      <div role="group">
        ...
        <button class="secondary" type="button" data-action="task-rule-reset">新建规则</button>
      </div>
    </form>
  `;
}
    ...
    <div>
      ${task.rules.map((rule) => `
        <article>
          ...
          <div role="group">
            <button class="outline" type="button" data-action="task-rule-edit" data-id="${rule.id}">编辑规则</button>
            <button class="outline" type="button" data-action="task-rule-delete" data-id="${rule.id}">删除规则</button>
          </div>
        </article>
      `).join("")}
    </div>
...
function renderContactSearch(app) {
  ...
  return `
    <form id="contact-search-form">
      ...
      <div role="group">
        <button type="submit">查询联系人</button>
      </div>
    </form>
...
export const tasksSection = {
  ...
  render(app) {
    ...
    if (app.state.route.secondary === "runs") {
      return `<div class="grid" style="grid-template-columns: repeat(12, minmax(0, 1fr)); align-items:start;">
        ${panel("当前任务", "默认跟随当前任务。", `<div>${currentTask(app) ? `当前任务: ${escapeHtml(currentTask(app).task_name)}` : "请先在任务配置中选中一个任务。"}</div>${taskSelector(app, "task-run-current", app.state.taskRunPage.taskId || app.state.selectedTaskId)}`, { span: 4 })}
