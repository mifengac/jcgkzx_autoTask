  ...
  return `
    <form id="result-filter-form">
      ...
      <div role="group">
        ...
        <button class="secondary" type="button" data-action="results-reset">重置筛选</button>
      </div>
    </form>
  `;
}
...
function renderResultTable(app) {
  ...
  const rows = page.items.map((item) => [
    ...
    `<span>${truncateText(item.oracle_eid, 36)}</span>`,
    truncateText(item.event_key, 28),
    `<div role="group"><button class="outline" type="button" data-action="open-detail" data-type="theme-result" data-id="${item.id}">详情</button></div>`,
  ]);
